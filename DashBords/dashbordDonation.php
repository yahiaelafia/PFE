...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($titre) && !empty($rib) && !empty($description) && !empty($association_id)) {
        move_uploaded_file($image_tmp, "../assets/" . $image);
        try {
            $stmt = $pdo->prepare("INSERT INTO post_donation (association_id, image, titre, rib, description) VALUES (:aid,:img,:titre,:rib,:desc)");
            $stmt->execute([':aid'=>$association_id,':img'=>$image,':titre'=>$titre,':rib'=>$rib,':desc'=>$description]);
            $message = "Le post de donation a été ajouté !";
...
<head>
    ...
    <title>Ajouter une donation — Ataa</title>
</head>
<body>
<main>
    <div class="fackimg">
    ...
    <div class="container">
        <h1><i class="fa-solid fa-hand-holding-heart" style="color:#2e7d32;"></i> Ajouter un post de donation</h1>
        <?php if ($message): ?>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <label>Titre</label>
            <input type="text" name="titre" placeholder="Titre de l'appel aux dons" required>
            <label>RIB</label>
