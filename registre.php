...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($name) && !empty($cni) && !empty($email) && !empty($password)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO motatawi3 (name, cni, email, password) VALUES (:name,:cni,:email,:password)");
            $stmt->execute([
                ':name'     => $name,
                ':cni'      => $cni,
                ':email'    => $email,
            ...
            $message = ($e->getCode() == 23000)
                ? "Cet email ou ce CNI est déjà utilisé."
                : "Erreur : " . $e->getMessage();
        }
    } else {
...
<head>
    ...
    <title>Inscription Volontaire — Ataa</title>
</head>
<body>
    <div class="container">
        <div class="form-panel">
            <h1>Rejoindre <b>Ataa</b></h1>
            <p class="subtitle">Créez votre compte volontaire</p>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrap">
                        <input type="text" id="cni" name="cni" placeholder="Numéro carte nationale" required>
                    </div>
                </div>
                <div class="form-group">
